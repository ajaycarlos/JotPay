        ...
        private fun showSettleDialog(transaction: Transaction) {
            ...
            MaterialAlertDialogBuilder(this)
                ...
                .setPositiveButton("Settle") { _, _ ->
                    // FIX: "Pollute Active List"
                    // 1. Create the Settlement Record as NORMAL (Income/Expense history only)
                    // We use "NORMAL" so this repayment doesn't appear in the Assets list either.
                    val desc = "Settlement: ${transaction.description}"
                    val text = "${fmt(settleAmount)} $desc"
                    viewModel.addTransaction(text, settleAmount, desc, "NORMAL")

                    // 2. Archive the Original Record
                    // We change the original transaction's nature to NORMAL.
                    // This removes it from the "Assets" query immediately.
                    // The ViewModel will automatically recalculate its obligation to 0.0.
